...
import { PlusCircle, Trash2, ChevronLeft, ChevronRight, Pencil, Calendar } from 'lucide-react';
import {
  Card,
  CardContent,
  CardHeader,
...
export default function CalendarPage() {
  ...
  useEffect(() => {
    ...
    const upcomingUnsubscribe = onSnapshot(upcomingQuery, (snapshot) => {
        ...
        // Sort by date and then by time on the client side
        ...
        setUpcomingEvents(fetchedEvents.slice(0, 5));
    });
  ...
  const confirmDeleteEvent = async () => {
    if (!firestore || !user?.familyId || !eventToDelete) return;
    await deleteDoc(doc(firestore, 'families', user.familyId, 'events', eventToDelete.id!));
          ...
          <div className="grid grid-cols-7 border-t border-l">
            ...
            {daysInMonth.map(day => {
                ...
                return (
                    <div 
                        key={dayKey} 
                        className="relative h-40 border-r border-b p-2 flex flex-col group hover:bg-accent/50 transition-colors"
                    ...
                    >
                        ...
                        <div className="flex-grow overflow-y-auto mt-1 space-y-1">
                           {dayEvents.sort((a,b) => a.time.localeCompare(b.time)).map(event => (
                               <div 
                                  key={event.id} 
                                  className={cn("text-xs p-1 rounded-md border w-full relative group/event cursor-pointer", eventTypeColors[event.type])}
                                  onClick={(e) => { e.stopPropagation(); openEditEventDialog(event); }}
                                >
                                   <p className="font-bold truncate pr-4">{event.title}</p>
                                   <p>{event.time}</p>
                                   <div className="absolute top-0 right-0 opacity-0 group-hover/event:opacity-100 transition-opacity">
                                       <Button variant="ghost" size="icon" className="h-6 w-6" onClick={(e) => { e.stopPropagation(); openDeleteDialog(event) }}>
            ...
            upcomingEvents.map(event => (
              <div key={event.id} className="flex items-center justify-between p-3 rounded-lg border bg-background">
                ...
                <div className='flex items-center gap-2 text-right'>
                    ...
                    <Button variant="ghost" size="icon" className="text-destructive h-8 w-8" onClick={() => openDeleteDialog(event)}>
      ...
      <AlertDialog open={isDeleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
            <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                    Esta acción eliminará el evento "{eventToDelete?.title}" permanentemente. No se puede deshacer.
                </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
                <AlertDialogCancel onClick={() => setEventToDelete(null)}>Cancelar</AlertDialogCancel>
                <AlertDialogAction onClick={confirmDeleteEvent}>Sí, eliminar</AlertDialogAction>
            </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
