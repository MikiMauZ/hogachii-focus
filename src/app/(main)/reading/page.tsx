...
import type { Reading, UserProfile } from '@/lib/types';
import {
  collection,
  onSnapshot,
  addDoc,
...
const ReadingDialog = ({
  ...
  useEffect(() => {
    if (itemToEdit) {
      ...
      setCurrentPage('');
      setStatus('to-read');
      setUserId(user?.uid || ''); // Default to current user
    }
  }, [itemToEdit, user]);

  const handleSave = async () => {
    if (!title || !userId || !firestore || !user?.familyId) return;
      ...
      <div className="grid gap-4 py-4">
        ...
        <div className="grid grid-cols-2 gap-4">
         ...
         <div className="grid grid-cols-2 gap-4">
            ...
            <div className="grid gap-2">
                <Label htmlFor="reader">Lector</Label>
                 <Select onValueChange={setUserId} value={userId}>
                    <SelectTrigger><SelectValue placeholder="Seleccionar lector"/></SelectTrigger>
                    <SelectContent>
                        {members.map(member => (
...
export default function ReadingPage() {
  ...
  const getMemberName = (userId: string) => {
    return members.find(m => m.uid === userId)?.displayName || 'Desconocido';
  }
                ...
                <Card key={item.id} className="flex flex-col">
                    ...
                    <CardContent className="flex-grow space-y-4">
                       ...
                       <div className="text-xs text-muted-foreground flex items-center gap-2 justify-center">
                            ...
                            <span>{getMemberName(item.userId)}</span>
                        </div>
                       {item.startDate && (
                           <div className="text-xs text-muted-foreground flex items-center gap-2 justify-center">
             ...
             {readingsByStatus['to-read'].map((item) => (
              <Card key={item.id} className="flex flex-col">
                ...
                <CardContent className="flex-grow">
                    <div className="text-xs text-muted-foreground flex items-center gap-2 justify-center">
                        <User className="h-4 w-4" />
                        <span>Para: {getMemberName(item.userId)}</span>
                    </div>
                </CardContent>
              ...
              <Card key={item.id} className="flex flex-col bg-muted/40">
                <CardHeader>
                 ...
                 <CardContent className="flex-grow space-y-2">
                     <div className="text-xs text-muted-foreground flex items-center gap-2 justify-center">
                        <User className="h-4 w-4" />
                        <span>Leído por: {getMemberName(item.userId)}</span>
                    </div>
                     {item.finishDate && (
                           <div className="text-xs text-muted-foreground flex items-center gap-2 justify-center">
                               <CalendarDays className="h-4 w-4" />
